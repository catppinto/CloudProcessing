    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
    {
        ...
        if (cloud_cluster->points.size () > 1000)
        {
            ...
            else
            {
                ...
                pcl::ModelCoefficients cc;
                geometry_msgs::PoseStamped pose_msg;
                pose_msg.header.stamp = ros::Time::now();
                cc = *coefficients_cylinder;
                pose_msg.pose.position.x = cc.values[0];
                pose_msg.pose.position.y = cc.values[1];
                pose_msg.pose.position.z = cc.values[2];

                pose_msg.pose.orientation.x = cc.values[3];
                pose_msg.pose.orientation.y = cc.values[4];
                pose_msg.pose.orientation.z = cc.values[5];
                pose_msg.pose.orientation.w = cc.values[6];

//                std::string child_frame = "cylinder_pose";
//                std::string source_frame = "camera_depth_optical_frame";
//                c.publishTfTagPose(pose_msg, child_frame, source_frame);

//                c.listenToTF(child_frame, "world");
...
int main(int argc, char** argv)
{
    ...
    if(argc > 1)
    {
        ...
        c.loadCloud("/home/cat/catkin_ws/src/tutorials/pcd_files/tablemug_1.pcd");
